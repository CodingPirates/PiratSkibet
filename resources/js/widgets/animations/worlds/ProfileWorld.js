...
export default class ProfileWorld extends World {
  static get defaultProps () {
    return {
      ...super.defaultProps,
      classNames: 'animation-world underwater-world underwater-world--middle',
      directorLimit: 3,
      crowdedness: 1
    }
  }
    ...
    return (
      <>
        <Director limit={this.directorLimit}>
          <AnimationGroup childNumber={5} scaleFactor={0.15} scaleFactorInterval={[120, 200]} durationFactor={120} top='15%' right='-15%' direction='left' randomizeInitial zIndex={6}>
            <BlueFishMid />
          </AnimationGroup>

          <AnimationGroup childNumber={4} scaleFactor={0.15} scaleFactorInterval={[120, 200]} durationFactor={130} top='20%' left='-15%' direction='right' randomizeInitial zIndex={6}>
            <BlueFishMid />
          </AnimationGroup>

          <AnimationGroup childNumber={6} scaleFactor={0.12} scaleFactorInterval={[100, 150]} durationFactor={180} bottom='10%' left='-12%' direction='right' randomizeInitial shadow zIndex={3}>
            <BlueFishMid />
          </AnimationGroup>

          <AnimationGroup childNumber={5} scaleFactor={0.12} scaleFactorInterval={[100, 150]} durationFactor={180} bottom='10%' left='-12%' direction='right' randomizeInitial shadow zIndex={3}>
            <BlueFishMid />
          </AnimationGroup>

          <Shark scaleFactor={0.18} durationFactor={150} direction='left' right='-20%' top='40%' zIndex={4} randomizeInitial />
          <Shark scaleFactor={0.18} durationFactor={160} direction='right' left='-20%' top='20%' zIndex={4} randomizeInitial />
